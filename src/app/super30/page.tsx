                ...
                {programs.length === 0 ? (
                    ...
                    <div className="grid gap-8 relative z-10">
                        {programs.map((program, index) => {
                            ...
                            const isClickable = isOpen || isLatest

                            const cardContent = (
                                <div className={`p-8 md:p-10 rounded-3xl bg-[#131315] border border-[#2E2E2E] transition-all duration-300 shadow-sm overflow-hidden relative ${
                                    isClickable 
                                        ? "hover:bg-[#18181b] hover:border-gold/30 hover:shadow-xl hover:shadow-gold/5 cursor-pointer group" 
                                    ...
                                    <div className="flex flex-col md:flex-row gap-8 justify-between relative z-10">
                                        <div className="flex-1">
                                            ...
                                            <h2 className={`text-3xl md:text-4xl font-bold mb-4 text-white tracking-tight transition-colors duration-300 ${isClickable ? "group-hover:text-gold" : ""}`}>
                                                {program.title}
                                            </h2>
                                        ...
                                        <div className="shrink-0 flex items-center md:items-end justify-between md:flex-col mt-4 md:mt-0 pt-6 md:pt-0 border-t md:border-t-0 border-white/10">
                                            ...
                                            {isClickable ? (
                                                <div className="inline-flex items-center gap-2 px-6 py-2.5 rounded-full bg-gold text-[#0b0b0c] font-bold text-sm group-hover:bg-gold-muted transition-all duration-300 shadow-md group-hover:shadow-gold/20">
                                                    Apply Now
                                                    <ArrowRight className="w-4 h-4 transform group-hover:translate-x-1 transition-transform duration-300" />
                                                </div>
                                            ) : (
                                                <div className="inline-flex items-center gap-2 px-6 py-2.5 rounded-full bg-white/5 text-gray-500 border border-white/10 font-medium text-sm cursor-not-allowed">
                                                    {program.isSoldOut ? "Sold Out" : "Registration Closed"}
                                                </div>
                                            )}
                                        </div>
                                    </div>
                            ...
                            return isClickable ? (
                                <Link href={`/super30/${program.slug.current}`} key={program._id} className="block group">
                                    {cardContent}
                                </Link>
                            ) : (
                                <div key={program._id} className="block">
                                    {cardContent}
                                </div>
                            )
                        })}
